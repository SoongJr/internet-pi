...
Other features:
  ...
  - **Temperature Probe Monitoring**: Tracks temperature reported by probes connected either via one-wire communication directly on the pi (e.g. [DS18B20](https://blog.robertelder.org/ds18b20-raspberry-pi-setup-pullup/)) or by external sensors exposed as prometheus clients in the network. Enable and configure using the `temp_probes*` variables in `config.yml`.
...
To find the dashboard, navigage to Dashboards, click Browse, then go to the Internet connection dashboard. If you star this dashboard, it will appear on the Grafana home page.
